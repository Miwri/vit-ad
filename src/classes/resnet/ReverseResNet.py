"""
ResNet Decoder implementation modified from https://github.com/JiahongChen/ResNet-decoder/
"""
...
class ReverseResNet(nn.Module):
    """CNN Decoder which mirrors ResNet Architecture to unfold an image folded by a ResNet.
    Decoder only supports image size 224
    """

    def __init__(
        self,
        block=Bottleneck,
        layers: list[int] = [3, 6, 4, 3],
        ...
        self.relu = nn.ReLU(inplace=True)
        self.tanh = nn.Tanh()
        self.upsample1 = nn.Upsample(size=7, mode="nearest")
        ...
        if zero_init_residual:
            for m in self.modules():
                ...
                # commented out because BasicBlock is only important for ResNet variants < 50
    ...
    def _forward_impl(self, x: Tensor, indices) -> Tensor:
        """Forward pass which returns a reconstructed image of size (3,224,224). Is called if indices for max unpooling are given"""
        ...
        # print(x.shape)
        x = self.unpool(x, indices, output_size=(112, 112))
        x = self.de_conv1(x)
        x = self.bn1(x)
        x = self.tanh(x)
        return x
    ...
    def _forward_cnns_only(self, x: Tensor) -> Tensor:
        """Forward pass which returns only the feature maps unfolded to 64 channels. Is called if no indices for max unpooling are given."""
        # See note [TorchScript super()]
        x = self.upsample1(x)
        x = self.layer4(x)
        x = self.layer3(x)
    ...
    def forward(self, x: Tensor, indices: Tensor = None) -> Tensor:
        """Forward pass to reconstruct an image.
        Args:
            x: Tensor, shape (BATCH_SIZE, 2048, 1, 1)
            indices: Tensor, shape (BATCH_SIZE, 64, 56, 56) to reverse max pooling from ResNet encoder
        """
        if indices is None:
